...
**The engineering work (writing code, running tests) does not require the server.** Only tracking calls (complete_skill, advance_phase, etc.) need it. If the server is down, continue the engineering work and catch up on tracking when it returns.

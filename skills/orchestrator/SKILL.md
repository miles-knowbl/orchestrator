...
|-----------|-------------|
| `../agentic-harness/references/loop-phases.md` | When executing engineering loop phases |
...
→ See `../agentic-harness/references/loop-phases.md` for detailed phase guidance (SCAFFOLD → IMPLEMENT → TEST → VERIFY → VALIDATE → DOCUMENT → REVIEW → SHIP), including rework patterns, phase checklists, and brownfield adaptations.
